...
public class ViewPagerIndicator extends LinearLayout {
     ...
     * 线的最大宽度
     */
    private final int DIMENSION_TRIANGEL_WIDTH = (int) (getScreenWidth() / 2);
    ...
    /**
     * 标题正常时的颜色（并没有使用  使用的是getContext().getResources().getColor(R.color.text_color)  ）
     */
    private static final int COLOR_TEXT_NORMAL = 0xFFC6CBCC;
    /**
     * 标题选中时的颜色
     */
    private static final int COLOR_TEXT_HIGHLIGHTCOLOR = 0xFFFF9A23;
...
//        mPaint.setColor(getContext().getResources().getColor(R.color.yellow));
        mPaint.setColor(GosDeploy.appConfig_Background());
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        Path path = new Path();
        int height = 6;
        path.moveTo(-(mInitTranslationX + mTranslationX), 0);
        path.lineTo(getScreenWidth(), 0);
        path.lineTo(getScreenWidth(), -height);
        path.lineTo(-(mInitTranslationX + mTranslationX), -height);
        path.close();
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mTriangleWidth = (int) (w / mTabVisibleCount);// 1/6 of
        // width
    ...
    protected void highLightTextView(int position) {
        ...
        if (view instanceof TextView) {
            ((TextView) view).setTextColor(getContext().getResources().getColor(R.color.text_color));
        }
    ...
    private void resetTextViewColor() {
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (view instanceof TextView) {
                ((TextView) view).setTextColor(getContext().getResources().getColor(R.color.text_state));
            }
        }
    }
    ...
    private TextView generateTextView(String text) {
        ...
        tv.setTextColor(getContext().getResources().getColor(R.color.text_state));
        tv.setText(text);
